...
def edit_employee(employee_id):
    ...
    if form.validate_on_submit():
        if not EmployeeService.get_emp_by_email(form.email.data) or form.email.data == employee.email:
            ...
            employee.department = DepartmentService.get_dep_by_name(form.department_name.data)
            employee.position = PositionService.get_pos_by_name(form.position_name.data)
            EmployeeService.save_changes()
            return redirect(url_for('m_bp.get_employees'))
        else:
            flash('Employee with this email {} already exists. Enter another email.'.format(form.email.data))
...
def add_employee():
    ...
    if form.validate_on_submit():
        if not EmployeeService.get_emp_by_email(form.email.data):
            EmployeeService.add_emp(form.first_name.data, form.last_name.data, form.hire_date.data, form.salary.data, form.email.data, form.department_name.data, form.position_name.data)
            return redirect(url_for('m_bp.get_employees'))
        else:
            flash('Employee with this email {} already exists. Enter another email.'.format(form.email.data))
